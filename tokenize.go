...
//	Run the parser on the given SQL string. The parser structure is passed in. An SQLITE_ status code is returned.
func (pParse *Parse) Run(zSQL string) (ErrMsg string, nErr int) {
	db := pParse.db
	...
	for i := 0; !db.mallocFailed && i < len(zSql); {
		...
		case TK_ILLEGAL:
			ErrMsg = fmt.Sprintf("unrecognized token: \"%v\"", &pParse.sLastToken)
			nErr++
			goto abort_parse
		case TK_SEMI:
			pParse.zTail = &zSql[i]
...
abort_parse:
	...
	if pParse.zErrMsg != "" {
		ErrMsg = pParse.zErrMsg
		sqlite3_log(pParse.rc, "%s", *ErrMsg)
		pParse.zErrMsg = ""
		nErr++
	}
