...
** error.
*/
func const void *sqlite3_errmsg16(sqlite3 *db){
  static const uint16 outOfMem[] = {
    'o', 'u', 't', ' ', 'o', 'f', ' ', 'm', 'e', 'm', 'o', 'r', 'y', 0
  };
  static const uint16 misuse[] = {
    'l', 'i', 'b', 'r', 'a', 'r', 'y', ' ', 
    'r', 'o', 'u', 't', 'i', 'n', 'e', ' ', 
    'c', 'a', 'l', 'l', 'e', 'd', ' ', 
    'o', 'u', 't', ' ', 
  ...
  if( db.mallocFailed ){
    ...
    ** ApiExit(), to avoid setting the database handle error message.
    */
...
static int createCollation(
  ...
  */
  ...
  pColl.pUser = pCtx;
  pColl.xDel = xDel;
  pColl.enc = (byte)(enc2 | (enc & SQLITE_UTF16_ALIGNED));
  db.Error(SQLITE_OK, "");
  return SQLITE_OK;
}
...
func int sqlite3_open16(
  ...
  *ppDb = 0;
  ...
  if( zFilename8 ){
    ...
    if( rc==SQLITE_OK && !ppDb.HasProperty(0, DB_SchemaLoaded) ){
      ppDb.SetEncoding(SQLITE_UTF16NATIVE)
    }
  }else{
    rc = SQLITE_NOMEM;
  }
...
func int sqlite3_create_collation16(
  ...
  db.mutex.Lock()
  assert( !db.mallocFailed );
  Name8 = sqlite3Utf16to8(db, Name, -1, SQLITE_UTF16NATIVE);
  if( Name8 ){
    rc = createCollation(db, Name8, (byte)enc, pCtx, xCompare, 0);
    Name8 = nil
  }
  rc = db.ApiExit(rc)
  db.mutex.Unlock()
