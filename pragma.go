...
# define SQLITE_INTEGRITY_CHECK_ERROR_MAX 100
  ...
  */
  if CaseInsensitiveMatch(zLeft, "encoding") {
    ...
    if( !zRight ){    /* "PRAGMA encoding" */
      ...
      sqlite3VdbeChangeP4(v, -1, encnames[pParse.db.Encoding()].Name, P4_STATIC);
      v.AddOp2(OP_ResultRow, 1, 1);
      ...
      */
      if( 
        !(db.HasProperty(0, DB_SchemaLoaded)) || 
        db.HasProperty(0, DB_Empty) 
      ){
        for(pEnc=&encnames[0]; pEnc.Name; pEnc++){
          if CaseInsensitiveMatch(zRight, pEnc.Name) {
            pParse.db.SetEncoding(pEnc.enc ? pEnc.enc : SQLITE_UTF16NATIVE)
            break;
          }
        }
        if( !pEnc.Name ){
          pParse.SetErrorMsg("unsupported encoding: %v", zRight);
        }
      }
    }
  }else
